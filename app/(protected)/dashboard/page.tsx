              ...
              {tasks.map((t, i) => {
                ...
                return (
                  <tr key={t.id} style={{ borderBottom: i < tasks.length - 1 ? '1px solid var(--color-border)' : undefined }}>
                    <td style={TD}>
                      <Link href={`/tasks/${t.id}`} style={ROW_LINK}>
                        {t.title}
                      </Link>
                    </td>
...
function RecentActivityFeed({ entries }: { entries: ActivityLogEntry[] }) {
  if (entries.length === 0) {
    return (
      <div style={{ padding: '12px 4px', borderRadius: 'var(--radius-control)', border: '1px dashed var(--color-border)', backgroundColor: 'var(--color-surface-subtle)' }}>
        <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', margin: 0 }}>No recent updates. Activity will appear as work moves.</p>
      </div>
    )
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column' }}>
      {entries.map((entry, i) => {
        ...
        return (
          ...
          >
            <div style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: 'var(--color-primary)', flexShrink: 0, marginTop: '5px', opacity: 0.55 }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-primary)', lineHeight: 1.35, margin: 0 }}>{description}</p>
...
export default async function DashboardPage() {
  ...
  if (role === 'member') {
    ...
    return (
      <div>
        <PageHeader title="My Dashboard" subtitle="Your tasks, deliverables, and payment status." className="mb-6" />
  ...
  if (role === 'reviewer') {
    ...
    return (
      <div>
        <PageHeader title="Review" subtitle="Queue and due items that need your sign-off." className="mb-6" />
        <ScopedKpiRow kpis={data.kpis} />
  ...
  if (role === 'coordinator') {
    ...
    const attentionCount =
      attention.overdueTasks.length + attention.blockedTasks.length + attention.revisionDeliverables.length + waiting.length

    return (
      <div>
        <PageHeader title="Coordinator" subtitle="Operational view of your assigned projects — workload, deadlines, and blockers." className="mb-6" />
        <ScopedKpiRow kpis={data.kpis} />
        {data.projectIds.length === 0 ? (
          <SectionCard title="Assignments">
            <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)', margin: 0 }}>No project assignments yet. When you are added to projects, this dashboard will populate.</p>
          </SectionCard>
        ) : (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '20px', marginBottom: '20px', alignItems: 'start' }}>
              <SectionCard
                ...
                actions={
                  attentionCount > 0 ? (
                    <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-muted)' }}>{attentionCount} items</span>
                  ) : undefined
                }
              >
                <AttentionQueue attention={attention} waitingClient={waiting} />
              </SectionCard>
              <SectionCard title="Recent activity" description="Latest changes in the workspace.">
  ...
  const [kpis, attention, pipeline, buckets, paymentSnapshot, waitingClient, activity, workload] = await Promise.all([
    getDashboardKpis(),
    getNeedsAttention(),
    getOpenTaskStatusCounts(),
    getDeadlineBuckets(),
  ...
  const statusLine =
    queueSignals === 0
      ? 'No critical blockers flagged in headline metrics today.'
      : `${queueSignals} headline risk signal${queueSignals === 1 ? '' : 's'} (overdue + client holds + in revision).`

  return (
    <div>
      <PageHeader title="Dashboard" subtitle="Owner / admin control center — pipeline health, deadlines, cash exposure, and team load." className="mb-2" />
      <p style={{ fontSize: '0.8125rem', color: 'var(--color-text-muted)', marginBottom: '28px', lineHeight: 1.45 }}>{statusLine}</p>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(168px, 1fr))', gap: '14px', marginBottom: '28px' }}>
        <KpiCard variant="dashboard" label="Active Projects" value={kpis.activeProjects} icon={<FolderKanban size={20} />} accent="primary" />
        <KpiCard
          variant="dashboard"
          label="Open Tasks"
        ...
        />
        <KpiCard variant="dashboard" label="Due This Week" value={kpis.dueThisWeek} icon={<Clock size={20} />} />
        <KpiCard variant="dashboard" label="Awaiting Review" value={kpis.awaitingReview} icon={<FileText size={20} />} />
        <KpiCard variant="dashboard" label="Waiting on Client" value={kpis.waitingClient} icon={<CalendarClock size={20} />} />
        <KpiCard variant="dashboard" label="In Revision" value={kpis.inRevision} icon={<RotateCcw size={20} />} accent={kpis.inRevision > 0 ? 'warning' : 'none'} />
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '20px', marginBottom: '20px', alignItems: 'start' }}>
        <SectionCard title="Operational health" description="Where open work sits in the pipeline — flow vs. friction.">
          <TaskStatusBarChart counts={pipeline} />
        </SectionCard>
        <SectionCard title="Deadline pressure" description="Tasks and projects with target dates in the next 14 days.">
          <DeadlineBucketsChart buckets={buckets} />
        </SectionCard>
      </div>
        ...
        <SectionCard
          ...
          actions={
            <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-muted)' }}>
              {attention.overdueTasks.length + attention.blockedTasks.length + attention.revisionDeliverables.length + waitingClient.length} surfaced
            </span>
          }
        >
          <AttentionQueue attention={attention} waitingClient={waitingClient} />
        </SectionCard>
        <SectionCard title="Payment snapshot" description="Exposure and status counts — not full finance.">
          <PaymentSnapshotCard snapshot={paymentSnapshot} />
        </SectionCard>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '20px', marginBottom: '20px', alignItems: 'start' }}>
        <SectionCard title="Team workload" description="Open assignments by person — overload is obvious.">
          <WorkloadBars users={workload} />
        </SectionCard>
        <SectionCard title="Recent activity" description="What changed recently.">
          <RecentActivityFeed entries={activity} />
        </SectionCard>
      </div>
    </div>
